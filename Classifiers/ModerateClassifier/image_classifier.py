...
transform_pipeline = transforms.Compose([
    ...
    transforms.RandomRotation(degrees=30),             # simulate camera angle change
    transforms.ColorJitter(brightness=0.5, contrast=0.5, saturation=0.5),
    transforms.GaussianBlur(kernel_size=11),            # expensive operation
    transforms.ToTensor(),
...
transform_pipeline = transforms.Compose([
    ...
    transforms.RandomRotation(degrees=30),             # simulate camera angle change
    transforms.ColorJitter(brightness=0.5, contrast=0.5, saturation=0.5),
    transforms.GaussianBlur(kernel_size=7),            # expensive operation
    transforms.ToTensor(),
    transforms.Normalize([0.485, 0.456, 0.406], [0.229, 0.224, 0.225])
])
